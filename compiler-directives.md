...
* `none` - disable reordering for values that follow
...
The returns directive `[[return=<option>]]` indicates to the compiler how the compiler should code generation regarding the `return` statement.
...
### The `file` / `line` directives

For generated source files, the file/line directives `[[file="<name>", line=<n>, increment=<n>]]` or `[[line=<n>, increment=<n>]]` indicates to the compiler the source is from a generated file. The declaration indicates the original source's line number so the generated output's line number can track the original source. The `increment` argument indicates how much to increment the counted line number of the original source file per output line found in the file being compiled.

The `line` directive can be declared without a `file` argument which will reuse the last `file` directive (which must be present at least once within the same Zax source file).

Other `source` directives found in a generated file will not presume the referenced source file was also generated. Thus the directives only apply to the file being parsed.
